...
export function classifyWithEngine(event, memory = [], customKeywords = null) {
  // Normalise customKeywords — must be {business:[], personal:[]}
  // Flat arrays (from a previous bug) would crash findMatches; fall back to defaults.
  let indicators = DEFAULT_INDICATORS;
  if (customKeywords && typeof customKeywords === 'object' && !Array.isArray(customKeywords)) {
    indicators = {
      business: Array.isArray(customKeywords.business) ? customKeywords.business : DEFAULT_INDICATORS.business,
      personal: Array.isArray(customKeywords.personal) ? customKeywords.personal : DEFAULT_INDICATORS.personal,
    };
  }
