...
export function classifyWithEngine(event, memory = [], customKeywords = null) {
  ...
  memory.forEach(past => {
    ...
    if (event.title.toLowerCase() === past.title.toLowerCase()) {
      maxSimilarity = 100;
      bestMatch = past;
    } else {
  ...
  // LAYER 3/4: Fallback to AI
  return {
    classification: null, // Needs AI
    confidence: 0,
    reasoning: 'Ambiguous patterns, requesting AI inference...',
    source: 'AI Inference',
    suggestedDestination
  };
}
