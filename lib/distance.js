...
/**
 * Calculate multiple round-trips in parallel on the server.
 * @param {string[]} destinations - Array of destination strings
 * @returns {Promise<Array>}
 */
export async function calculateBulkRoundTrips(destinations) {
  if (!destinations || !Array.isArray(destinations)) return [];
  
  // Use Promise.all to fetch all distances in parallel on the server
  // This is much faster than sequential fetches and safer for mobile browsers
  const results = await Promise.all(
    destinations.map(async (dest) => {
      try {
        if (!dest) return null;
        return await calculateRoundTrip(dest);
      } catch (err) {
        console.error(`Bulk distance failure for ${dest}:`, err.message);
        return null; // Return null for failed items to keep the array mapping
      }
    })
  );
